...
%recupera todos los actores y actrices

actores(M,A):- actor(M,A,_);actress(M,A,_). 

%Encontrar dos actores o actrices que hallan trabajado juntos en mas de una pelicula

masUnActorMasUnaPelicula(A1,A2,M1):- movie(M1,_),movie(M2,_),actores(M1,A1),actores(M2,A2),M1\=M2,A1\=A2.

%Todos los actores y actrices que hallan trabajado para un mismo director

mismoDirector(A1,A2,D):- director(M,D),actores(M,A1),actores(M,A2),A1@<A2.		
listaMismoDirector(Lista):- findall(A1, (director(M,_),actores(M,A1)) , Lista). %con una lista de los actores

%Encontrar el actor o actriz con mas actuaciones en un año

member(X,[X|_]).                       
member(X,[_|L]):- member(X,L).

actuacionesUnAño(A):- movie(M1,Y),movie(M2,Y),actores(M,A).
masActuacionesUnAño(Lista):- findall(A, (actores(M,A),movie(M1,Y),movie(M2,Y)) , Lista ). 

%Encontrar el director que mantiene al mismo cuadro, es decir el mismo grupo de actores/actrices entre dos peliculas distintas

mismoCuadro(D):- director(M1,D),director(M2,D),actores(M1,A1),actores(M2,A2),M1\=M2,A1\=A2.